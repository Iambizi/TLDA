...
export default async function ParticipantsPage({ searchParams }: ParticipantsPageProps) {
  ...
  if (statusFilter && Object.keys(APPLICATION_STATUS_LABELS).includes(statusFilter)) {
    ...
    <div>
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm" style={{ color: 'var(--muted)' }}>
            {formattedApplications.length} total applicant{formattedApplications.length === 1 ? '' : 's'} found.
          </p>
        </div>
      </div>
      ...
      <div className="mb-6 flex gap-2 overflow-x-auto pb-2">
        <Link
          ...
          style={{
            background: !statusFilter ? 'var(--neutral-200)' : 'var(--neutral-100)',
        ...
        {Object.entries(APPLICATION_STATUS_LABELS).map(([key, label]) => {
          ...
          const colorClass = APPLICATION_STATUS_COLORS[key as keyof typeof APPLICATION_STATUS_LABELS] || 'bg-neutral-100 text-neutral-600'
          return (
            <Link
              key={key}
              href={`/participants?status=${key}`}
              className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors whitespace-nowrap ${isActive ? colorClass : 'bg-neutral-100 text-neutral-600'}`}
              style={{
      ...
      <div className="rounded-2xl border overflow-hidden" style={{ background: 'var(--surface)', borderColor: 'var(--border)' }}>
        {formattedApplications.length === 0 ? (
          <div className="p-12 text-center">
            <p className="text-sm" style={{ color: 'var(--muted)' }}>No participants found matching the current filters.</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y" style={{ borderColor: 'var(--border)' }}>
                {formattedApplications.map((app: any) => {
                  ...
                  return (
                    <tr key={app.id} className="hover:bg-neutral-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4" style={{ color: 'var(--neutral-600)' }}>
                        <span className="truncate max-w-[200px] block" title={app.contact}>
                          {app.contact}
                        </span>
                      </td>
                      <td className="px-6 py-4" style={{ color: 'var(--neutral-600)' }}>
                      ...
                      <td className="px-6 py-4 text-right tabular-nums" style={{ color: 'var(--muted)' }}>
                        {date}
                      </td>
                    </tr>
                  )
